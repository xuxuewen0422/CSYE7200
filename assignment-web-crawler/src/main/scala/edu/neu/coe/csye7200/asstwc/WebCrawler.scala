...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[Try[URL]] =for (
      nodea <- ns \\ "a"; nodehf <- nodea \ "@href"; nodehf2 <- nodehf.head;
      res = nodehf2.toString ) yield createURL(Some(u), res)
    // TO BE IMPLEMENTED
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    val us2 = us.distinct take 10; val us3 = for {u <- us2} yield wget(u); val us4 = MonadOps.mapFuture(us3)
    Future.sequence(us4)
